...
namespace ex07_EmployeeMngApp.ViewModels
{
    public class MainViewModel : Conductor<object>
    {
        ...
        /// </summary>
        public async void SaveEmployee() // save버튼 // 이게 클릭이벤트처럼 동작하네..? 
        { // 다이어로그  디자인 때문에 async추가
            // MessageBox.Show("저장버튼 동작!");

            if (Common.DialogCoordinator != null)  // 이거도 다이얼로그때문에 추가
            {
        ...
        public async void DelEmployee() // 삭제
        {
            if (Common.DialogCoordinator != null)  // 이거도 다이얼로그때문에 추가
            {
                this._dialogCoordinator = Common.DialogCoordinator;
